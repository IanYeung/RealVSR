...
def read_img_seq(path, color='RGB'):
    """
    ...
    img_l = channel_convert(img_l[0].shape[2], color, img_l)
    # stack to Torch tensor
    imgs = np.stack(img_l, axis=0)
    imgs = imgs[:, :, :, [2, 1, 0]] if imgs.shape[-1] == 3 else imgs
    imgs = torch.from_numpy(np.ascontiguousarray(np.transpose(imgs, (0, 3, 1, 2)))).float()
    return imgs


def _read_raw_lmdb(env, key, size):
    """
    read image from lmdb with key (w/ and w/o fixed size)
    size: (C, H, W) tuple
    """
...
def read_raw(env, path, size=None):
    """
    read image by np or from lmdb
    return: Numpy float32, HWC, C=4, [0,1]
    """
    if env is None:  # img
        img = np.load(path)
    else:
        img = _read_raw_lmdb(env, path, size)
    return img


def read_raw_seq(path):
    """
    Read a sequence of images from a given folder path
    Args:
        path (list/str): list of image paths/image folder path
    Returns:
        imgs (Tensor): size (T, C, H, W), C=4, [0, 1]
    """
    if type(path) is list:
        img_path_l = path
    else:
        img_path_l = sorted(glob.glob(os.path.join(path, '*')))
    img_l = [read_raw(None, v) for v in img_path_l]
    # stack to Torch tensor
    imgs = np.stack(img_l, axis=0)
    imgs = torch.from_numpy(np.ascontiguousarray(np.transpose(imgs, (0, 3, 1, 2)))).float()
    return imgs
...
def UnMaskFilterGaussian(img):

    radius = random.randint(3, 15)
    percent = random.randint(30, 110)
    threshold = 0

    pimg = Image.fromarray(img)
    dimg = pimg.filter(ImageFilter.UnsharpMask(radius=radius, percent=percent, threshold=threshold))

    return np.array(dimg)


def UnMaskFilterBilateral(img):

    d = random.randint(3, 9)
    sigmacolor = random.randint(150, 300)
    sigmaspace = random.randint(150, 300)
    percent = random.randint(100, 210)
    threshold = 0
...
def UnMaskFilterGD(img):
    if random.random() > 0.5:
        return UnMaskFilterGaussian(img)
    else:
        return UnMaskFilterBilateral(img)


def sharpen_gt(img, threshold=1.0):
    if random.random() < threshold:
        return UnMaskFilterGD(img)
    else:
        return img
