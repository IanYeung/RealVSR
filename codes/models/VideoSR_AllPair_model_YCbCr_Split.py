...
class VideoSRModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            #### loss
            loss_type = train_opt['pixel_criterion_y']
            if loss_type == 'l1':
                self.cri_pix_y = nn.L1Loss(reduction='mean').to(self.device)
            elif loss_type == 'l2':
                self.cri_pix_y = nn.MSELoss(reduction='mean').to(self.device)
            elif loss_type == 'cb':
                self.cri_pix_y = CharbonnierLoss(reduction='mean').to(self.device)
            elif loss_type == 'hb':
                self.cri_pix_y = HuberLoss(reduction='mean').to(self.device)
            elif loss_type == 'gw':
                self.cri_pix_y = GWLoss(w=4, reduction='mean').to(self.device)
            elif loss_type == 'pyr':
                self.cri_pix_y = PyramidLoss(num_levels=3, pyr_mode='gau', loss_mode='cb', reduction='mean')
            elif loss_type == 'lappyr':
                self.cri_pix_y = LapPyrLoss(num_levels=3, lf_mode='ssim', hf_mode='cb', reduction='mean')
            elif loss_type == 'msssim':
                channels = opt['network_G']['nc']
                self.cri_pix_y = MS_SSIM(channels=channels).to(self.device)
            else:
                raise NotImplementedError('Loss type [{:s}] is not recognized.'.format(loss_type))
            self.l_pix_w_y = train_opt['pixel_weight_y']
            ...
            if loss_type == 'l1':
                ...
                self.cri_pix_c = PyramidLoss(num_levels=3, pyr_mode='gau', loss_mode='cb', reduction='mean')
            elif loss_type == 'lappyr':
                self.cri_pix_c = LapPyrLoss(num_levels=3, lf_mode='ssim', hf_mode='cb', reduction='mean')
            elif loss_type == 'msssim':
                channels = opt['network_G']['nc']
                self.cri_pix_c = MS_SSIM(channels=channels).to(self.device)
            else:
                raise NotImplementedError('Loss type [{:s}] is not recognized.'.format(loss_type))
    ...
    def optimize_parameters(self, step):
        ...
        l_pix_y = self.l_pix_w_y * self.cri_pix_y(self.fake_H[:, 0:1, :, :], self.var_H[:, center_idx, 0:1, :, :].contiguous())
        # loss for color channel
        l_pix_c = self.l_pix_w_c * self.cri_pix_c(self.fake_H[:, 1:3, :, :], self.var_H[:, center_idx, 1:3, :, :].contiguous())
        l_pix = l_pix_y + l_pix_c
        l_pix.backward()
        self.optimizer_G.step()
        # set log
        self.log_dict['l_pix_y'] = l_pix_y.item()
        self.log_dict['l_pix_c'] = l_pix_c.item()
