...
class EDVR(nn.Module):
    def __init__(self, nf=64, nc=3, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        if self.is_predeblur:
            ...
            if self.HR_in:
                self.conv_first_1 = nn.Conv2d(nc, nf, 3, 1, 1, bias=True)
                self.conv_first_2 = nn.Conv2d(nf, nf, 3, 2, 1, bias=True)
                self.conv_first_3 = nn.Conv2d(nf, nf, 3, 2, 1, bias=True)
            else:
                self.conv_first = nn.Conv2d(nc, nf, 3, 1, 1, bias=True)
        ...
        self.conv_last = nn.Conv2d(64, nc, 3, 1, 1, bias=True)
...
class EDVR_NoUp(nn.Module):
    def __init__(self, nf=64, nc=3, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
                 predeblur=False, HR_in=False, w_TSA=True):
        super(EDVR_NoUp, self).__init__()
        self.nf = nf
        self.nc = nc
        self.center = nframes // 2 if center is None else center
        self.is_predeblur = True if predeblur else False
...
if __name__ == '__main__':
    EDVR_M = EDVR(nf=64, nc=1, nframes=7, groups=8, front_RBs=5, back_RBs=10, center=None,
                  predeblur=False, HR_in=False, w_TSA=False)
    params = sum([p.data.nelement() if p.requires_grad else 0 for p in EDVR_M.parameters()])
    print('EDVR(M) Parameter count: {}'.format(params))

    EDVR_WoUp = EDVR_NoUp(nf=64, nc=1, nframes=7, groups=8, front_RBs=5, back_RBs=10, center=None,
                          predeblur=False, HR_in=False, w_TSA=False)
    params = sum([p.data.nelement() if p.requires_grad else 0 for p in EDVR_M.parameters()])
    print('EDVR(NoUp) Parameter count: {}'.format(params))
